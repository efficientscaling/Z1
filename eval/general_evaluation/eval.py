...
from reasoning import ThinkingLLM
...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size of problems")
    ...
    parser.add_argument("--max_thinking_tokens", default=4096, type=int)
...
def infer(args):
    ...
    llm = ThinkingLLM(model=model_name_or_path, 
    ...
    for cur_generation_epoch in range(generation_epoch):
        ...
        # completions=[]
        # problems_chunked = list(chunked(list(prompt_batch), args.batch_size))
        # for problems in tqdm(problems_chunked):
        #     completions += llm.generate(problems, sampling_params)
        #     print(completions[-1])
        # completions = llm.generate(prompt_batch, sampling_params)
        _, completions = llm.thinking_generate(prompt_batch, sampling_params, max_tokens_for_thinking=args.max_thinking_tokens)
